...
# All 75 companies with their career page URLs and ATS type
COMPANIES = [
...
def fetch_greenhouse_jobs(company):
    """Fetch jobs from Greenhouse ATS boards"""
    try:
        api_url = company["url"].replace("job-boards.greenhouse.io", "boards-api.greenhouse.io/v1/boards") + "/jobs?content=true"
...
def fetch_ashby_jobs(company):
    """Fetch jobs from Ashby ATS boards"""
    try:
...
def fetch_generic_jobs(company):
    """Scrape generic career pages using BeautifulSoup"""
    try:
        ...
        for a in soup.find_all("a", href=True):
            ...
            if text and len(text) > 5 and any(kw in href.lower() for kw in ["/job", "/position", "/opening", "/role"]):
...
def send_email(new_jobs_by_company):
    """Send email notification about new job postings"""
    sender = os.environ.get("GMAIL_USER")
    password = os.environ.get("GMAIL_APP_PASSWORD")

    if not sender or not password:
        print("[ERROR] GMAIL_USER or GMAIL_APP_PASSWORD not set. Skipping email.")
        return

    total = sum(len(jobs) for jobs in new_jobs_by_company.values())
    subject = f"🚀 {total} New Job Opening{'s' if total > 1 else ''} Found! [{datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')}]"
    ...
    for company, jobs in new_jobs_by_company.items():
        for job in jobs:
            rows += f"""
            <tr>
                <td style="padding:8px;border:1px solid #ddd;font-weight:bold;color:#1a1a2e">{company}</td>
                ...
                <td style="padding:8px;border:1px solid #ddd">
                    <a href="{job['url']}" style="color:#1155CC;text-decoration:none">Apply →</a>
                </td>
            </tr>"""

    html = f"""
    <html><body style="font-family:Arial,sans-serif;max-width:800px;margin:auto;padding:20px">
        <div style="background:linear-gradient(135deg,#1F4E79,#2E86AB);padding:25px;border-radius:10px;color:white;text-align:center;margin-bottom:20px">
            <h1 style="margin:0;font-size:24px">🚀 New Job Alerts</h1>
            <p style="margin:8px 0 0;opacity:0.9">{total} new opening{'s' if total > 1 else ''} found across your tracked companies</p>
        </div>
        <table style="width:100%;border-collapse:collapse;margin-bottom:20px">
            <thead>
                <tr style="background:#1F4E79;color:white">
                    <th style="padding:10px;border:1px solid #ddd;text-align:left">Company</th>
                    <th style="padding:10px;border:1px solid #ddd;text-align:left">Job Title</th>
        ...
        <p style="color:#666;font-size:12px;text-align:center">
            Checked at {datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')} • Job Tracker Bot
        </p>
    </body></html>
    """
    ...
    try:
        ...
        print(f"[EMAIL] Sent notification for {total} new job(s) to {NOTIFY_EMAIL}")
    except Exception as e:
...
def main():
    ...
    if new_jobs_by_company:
        total = sum(len(j) for j in new_jobs_by_company.values())
        print(f"\n[SUMMARY] {total} new job(s) found. Sending email...")
        send_email(new_jobs_by_company)
    else:
        print("\n[SUMMARY] No new jobs found this run.")

    print(f"\nDone. Total tracked jobs: {len(seen)}")
